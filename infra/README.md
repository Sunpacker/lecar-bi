...
4. В GitHub Environment `Production` настроить variables: `VPS_HOST`, `VPS_PORT`, `VPS_USER=deploy`, `VPS_PATH=/opt/lecar-bi`, `VPS_PROJECT=lecar-bi`, `VPS_ENV_FILE=/opt/lecar-bi/infra/.env`, `VPS_BACKUP_DIR=/opt/lecar-bi/backups`. SSH-порт задаётся явно. Environment secrets: `VPS_SSH_KEY`, `VPS_KNOWN_HOSTS`. Job `deploy-vps` явно привязан к `Production`, иначе эти значения недоступны.
