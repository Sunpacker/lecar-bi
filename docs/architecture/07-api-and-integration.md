...
1. **Контракт OpenAPI (`contracts/openapi/analytics-v1.yaml`):**
   - Все защищённые endpoints аннотированы расширением `x-required-capability`, фиксирующим обязательное атомарное право для выполнения операции:
     ...
     - `support.use`: приватные диалоги поддержки, сообщения, generation stream/retry и feedback;
   ...
   - Browser вызывает support API только через Next.js `/api/support/*`. BFF проверяет подписанную
     session и добавляет server-only `X-Support-BFF-Key`; analytics support routes отклоняют прямой
     запрос с одним `X-User-Id`.
2. **Backend Enforcement:**
   ...
   - Support API повторно проверяет актуальное членство, capability и ownership в Application/Persistence;
     все 10 операций описаны в OpenAPI, а SSE передаёт только сохранённые snapshots.
