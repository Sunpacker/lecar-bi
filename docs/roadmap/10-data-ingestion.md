...
1. **Frontend Data Ingestion UI:**
   - Страница `/imports` со списком загруженных батчей, статусами и прогресс-барами.
   - Модальное окно или зона загрузки CSV-файла (drag & drop) с выбором типа датасета (`sales`, `inventory`).
   - Детальный просмотр батча: статистика строк (всего, успешно, ошибок), таблица ошибок валидации (номер строки, поле, текст ошибки).
   - Кнопка повторного запуска (`Retry`) для ошибочных батчей.
   - Интеграция с OpenAPI клиентом.
2. **Интеграционный checkpoint фазы 10:**
...
- Реализовать Frontend UI для Data Ingestion (Task 2 текущей фазы).
